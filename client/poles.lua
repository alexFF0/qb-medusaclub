...
CreateThread(function()
	for k, v in pairs(Config.Poles) do
		...
		exports['qb-target']:AddBoxZone("UniDance"..k, v.coords.xyz, 0.7, 0.7, { name="UniDance"..k, heading = v.coords.w, debugPoly=Config.Debug, minZ = v.minZ, maxZ = v.maxZ, },
			{ options = { { event = "qb-medusaclub:Dance", icon = "fas fa-chair", label = Loc[Config.Lan].info["dance"], job = "medusa", loc = v.coords, pole = v.pole }, },
				distance = v.distance })
	end
	for k, v in pairs(Config.Dance) do
		Poles["UniPvtDance"..k] =
			exports['qb-target']:AddBoxZone("UniPvtDance"..k, v.coords.xyz, 0.7, 0.7, { name="UniPvtDance"..k, heading = v.coords.w, debugPoly=Config.Debug, minZ = v.minZ, maxZ = v.maxZ, },
				{ options = { { event = "qb-medusaclub:PvtDance", icon = "fas fa-chair", label = Loc[Config.Lan].info["dance"], job = "medusa", loc = v.coords, }, },
					distance = 1.7 })
	end
end)

RegisterNetEvent('qb-medusaclub:Dance', function(data)
	local canDance = true
	local dancing = false
...
RegisterNetEvent('qb-medusaclub:PvtDance', function(data)
	local canDance = true
	local dancing = false
