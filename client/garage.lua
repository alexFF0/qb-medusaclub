...
CreateThread(function()
	for k, v in pairs(Config.Locations) do
		if v.garage then
			...
			Targets["VanGarage: "..k] =
				exports['qb-target']:AddBoxZone("VanGarage: "..k, vector3(out.x, out.y, out.z-1.03), 0.8, 0.5, { name="VanGarage: "..k, heading = out[4]+180.0, debugPoly=Config.Debug, minZ=(out.z-1.03)-0.1, maxZ=(out.z-1.03)+1.3 },
					{ options = { { event = "qb-medusaclub:client:Garage:Menu", icon = "fas fa-clipboard", label = Loc[Config.Lan].targetinfo["job_vehicles"], job = v.job, coords = v.garage.spawn, list = v.garage.list }, },
					distance = 2.0 })
			parking[#parking+1] = makeProp({ prop = `prop_parkingpay`, coords = vector4(out.x, out.y, out.z-1.03, out.w+180.0)}, 1, false)
		end
	end
end)

local currentVeh = { out = false, current = nil }
RegisterNetEvent('qb-medusaclub:client:Garage:Menu', function(data)
	...
	vehicleMenu[#vehicleMenu + 1] = { icon = "fas fa-circle-xmark", header = "", txt = Loc[Config.Lan].menu["close"], params = { event = "qb-medusaclub:client:Menu:Close" } }
	if currentVeh.out and DoesEntityExist(currentVeh.current) then
		vehicleMenu[#vehicleMenu+1] = { icon = "fas fa-clipboard-list", header = Loc[Config.Lan].menu["out_of_garage"],
										...
										params = { event = "qb-medusaclub:client:Garage:Blip", }, }
		vehicleMenu[#vehicleMenu+1] = { icon = "fas fa-car-burst", header = "Remove Vehicle", params = { event = "qb-medusaclub:client:RemSpawn" } }
	else
		...
		for _, v in pairs(data.list) do
			...
			vehicleMenu[#vehicleMenu+1] = { header = name, params = { event = "qb-medusaclub:client:SpawnList", args = { spawnName = spawnName, coords = data.coords } } }
		end
	end
    exports['qb-menu']:openMenu( vehicleMenu)
end)

RegisterNetEvent("qb-medusaclub:client:SpawnList", function(data)
...
RegisterNetEvent("qb-medusaclub:client:RemSpawn", function(data)
