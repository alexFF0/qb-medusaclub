...
AddEventHandler('onResourceStart', function(r) if GetCurrentResourceName() ~= r then return end
	...
	if not QBCore.Shared.Jobs["medusa"] then print("^5Debug^7: ^1Error^7: ^2Job role not found ^7- '^6medusa^7'") end
end)
...
--CreateThread(function()
   ...
   -- for _, v in pairs(cocktails) do QBCore.Functions.CreateUseableItem(v, function(source, item) TriggerClientEvent('qb-medusaclub:client:DrinkAlcohol', source, item.name) end) end
--
--	local beers = { "ambeer", "dusche", "logger", "pisswasser", "pisswasser2", "pisswasser3" }
--for _, v in pairs(beers) do QBCore.Functions.CreateUseableItem(v, function(source, item) TriggerClientEvent('qb-medusaclub:client:DrinkAlcohol', source, item.name) end) end

--	local drinks = { "sprunk", "sprunklight", "ecola", "ecolalight", "cranberry", "pinejuice" }
--    for _, v in pairs(drinks) do QBCore.Functions.CreateUseableItem(v, function(source, item) TriggerClientEvent('qb-medusaclub:client:Drink', source, item.name) end) end

--	local food = { "chocolate", "vusliders", "vutacos", "nplate", "tots", "nachos", "crisps" }
 --   for _,v in pairs(food) do QBCore.Functions.CreateUseableItem(v, function(source, item) TriggerClientEvent('qb-medusaclub:client:Eat', source, item.name) end) end
--end)

QBCore.Functions.CreateCallback('qb-medusaclub:GetCash', function(source, cb)
	cb(QBCore.Functions.GetPlayer(source).Functions.GetMoney("cash"))
end)
RegisterServerEvent("qb-medusaclub:StrepTip", function()
	QBCore.Functions.GetPlayer(source).Functions.RemoveMoney("cash", Config.TipCost)
end)

---Crafting
RegisterServerEvent('qb-medusaclub:Crafting:GetItem', function(ItemMake, craftable)
	local src = source
	...
	if craftable then
		...
		for k, v in pairs(craftable[ItemMake]) do TriggerEvent("qb-medusaclub:server:toggleItem", false, tostring(k), v, src) end
	end
	--This should give the item, while the rest removes the requirements
	TriggerEvent("qb-medusaclub:server:toggleItem", true, ItemMake, amount, src)
end)

-- Server Side Built in DJ Booth stuff
local xSound = exports.xsound
RegisterNetEvent('qb-medusaclub:server:playMusic', function(song, zoneNum)
    local src = source
    ...
    TriggerClientEvent('qb-medusaclub:client:playMusic', src, { zone = zoneNum })
end)
RegisterNetEvent('qb-medusaclub:server:stopMusic', function(data)
    local src = source
    ...
    TriggerClientEvent('qb-medusaclub:client:playMusic', src, { zone = data.zoneNum })
end)
RegisterNetEvent('qb-medusaclub:server:pauseMusic', function(data)
    local src = source
	local zoneLabel = Config.Locations[data.zoneNum].job..data.zoneNum
    if Config.Locations[data.zoneNum].Booth.playing then
        Config.Locations[data.zoneNum].Booth.playing = false
...
RegisterNetEvent('qb-medusaclub:server:resumeMusic', function(data)
    local src = source
	local zoneLabel = Config.Locations[data.zoneNum].job..data.zoneNum
...
RegisterNetEvent('qb-medusaclub:server:changeVolume', function(volume, zoneNum)
    local src = source
	local Booth = Config.Locations[zoneNum].Booth
	local zoneLabel = Config.Locations[zoneNum].job..zoneNum
...
RegisterNetEvent('qb-medusaclub:server:toggleItem', function(give, item, amount, newsrc)
	...
	if give == 0 or give == false then
		...
		else TriggerEvent("qb-medusaclub:server:DupeWarn", item, src) end -- if not boot the player
	else
...
RegisterNetEvent("qb-medusaclub:server:DupeWarn", function(item, newsrc)
	local src = newsrc or source
